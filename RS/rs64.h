...
extern "C" {
	...
	//Функции из RS64.c
	/* InitRS заполняет таблицы SSE, и выбирает версию кодера/декодера
	 * Если поддерживается AVX2, будет использоваться эта версия
	 ...
	 *	-4, если при значение какой-либо ошибки получается равным 0
